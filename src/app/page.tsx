...
import { Shield, Users, Radio, ShieldAlert, AlertTriangle, UserPlus, Medal, Activity, Check, PhoneCall, LayoutDashboard, Lock, User, Smartphone, ThumbsUp, ThumbsDown, Filter, ArrowUpDown, Flame, Bomb, Wind, Sun, Moon, Eye, CheckCircle, Key, Settings, Truck, Mail, ArrowRight, BarChart3, Globe, Monitor, ShieldCheck, Copy, ChevronDown, Home, Headphones, Upload } from 'lucide-react';
...
export default function CrisisManagementSystem() {
  ...
  const [currentView, setCurrentView] = useState<'home' | 'report' | 'volunteer' | 'admin' | 'support' | 'admin-edit' | 'analytics'>('home');
  ...
  const navigateToView = (viewName: 'home' | 'report' | 'volunteer' | 'admin' | 'support' | 'admin-edit' | 'analytics') => {
  ...
  const handleSendReportOTP = async (phone: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("🚀 کد تایید به حساب بله شما ارسال شد.");
      } else {
        alert("❌ خطا در اتصال به سرور ارسال پیامک.");
      }
    } catch (err) {
      alert("❌ خطا در شبکه.");
    }
  ...
  const handleSendAuthOTP = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAuthOtpSent(true);
        alert("🚀 کد یکبار مصرف به حساب بله شما ارسال شد.");
      } else {
  ...
  const handleIncidentSubmit = async (e: React.FormEvent) => {
    ...
    // 📍 ولیدیشن و بررسی عدم تغییر لوکیشن از حالت پیش‌فرض (میدان آزادی تهران)
    ...
    // بازگرداندن نقشه به حالت پیش‌فرض پس از ثبت موفقیت‌آمیز
    ...
    <main dir="rtl" className={`relative w-full h-screen flex flex-col md:flex-row overflow-hidden font-sans select-none antialiased transition-colors duration-300 ${darkMode ? 'bg-slate-900 text-slate-100' : 'bg-[#e0f2fe]'}`}>
      ...
      {currentView !== 'support' && (
        <div className="fixed left-4 bottom-20 md:left-8 md:bottom-8 z-[480] flex flex-col items-center group">
          <button
            type="button"
            onClick={() => { navigateToView('support'); setIsAuthMode(false); setSelectedVolForPage(null); }}
            className={`p-4 rounded-full shadow-[0_10px_40px_rgba(0,0,0,0.3)] transition-all duration-300 hover:scale-110 active:scale-95 flex items-center justify-center ${
              darkMode 
                ? 'bg-slate-900 text-red-400 border border-slate-800 hover:border-red-500/50 shadow-[0_0_25px_rgba(239,68,68,0.15)]' 
                : 'bg-white text-red-600 border border-slate-200 hover:border-red-300 shadow-[0_8px_30px_rgba(0,0,0,0.12)]'
            }`}
            title="خانه"
          >
            <Headphones className={`w-5 h-5 md:w-6 md:h-6 ${currentView === 'home' ? 'animate-bounce' : 'animate-pulse'}`} />
          </button>
          <span className={`absolute top-full mt-2 whitespace-nowrap text-[10px] font-black px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none shadow ${darkMode ? 'bg-slate-955 text-slate-300' : 'bg-slate-800 text-white'}`}>📞 پشتیبانی فنی ستاد</span>
        </div>
      )}
      ...
      <aside className={`hidden md:flex absolute top-0 right-0 w-24 hover:w-80 h-full border-l justify-between flex-col z-[405] shadow-[-5px_0_40px_rgba(0,0,0,0.15)] transition-all duration-300 cubic-bezier(0.4, 0, 0.2, 1) group overflow-hidden ${darkMode ? 'bg-slate-900/95 border-slate-800' : 'bg-gradient-to-b from-pink-50 to-blue-50 border-slate-300 text-slate-900 shadow-xl'}`}>
        <div className="w-full h-full p-4 flex flex-col items-center justify-start gap-6 overflow-hidden">
          ...
          <nav className="w-full flex flex-col items-center justify-start gap-3 min-w-[240px]">
            ...
            <button onClick={() => { setSelectedVolForPage(null); if (isAdminLoggedIn || isHamyaatLoggedIn) { navigateToView('admin'); setIsAuthMode(false); } else { setIsAuthMode(true); } }} className={`flex w-full items-center px-4 py-3 rounded-2xl text-xs font-black transition-all duration-200 text-right ${currentView === 'admin' || isAuthMode ? 'bg-red-500 text-white scale-105' : 'text-slate-500 hover:bg-black/5 font-black'}`}>
      ...
      <div className={`md:hidden fixed bottom-0 left-0 right-0 h-16 border-t z-[490] flex items-center justify-around px-1 shadow-[0_-4px_20px_rgba(0,0,0,0.4)] ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 text-slate-800'}`}>
        ...
        <button onClick={() => { setSelectedVolForPage(null); if (isAdminLoggedIn || isHamyaatLoggedIn) { navigateToView('admin'); setIsAuthMode(false); } else { setIsAuthMode(true); } }} className={`flex flex-col items-center justify-center p-1 rounded-xl transition ${currentView === 'admin' || isAuthMode ? 'text-red-400 scale-110 font-bold' : 'text-slate-400'}`}>
      ...
      <section className="flex-1 h-full relative flex flex-col overflow-hidden pb-20 md:pb-0 md:pr-24">
        ...
        {currentView === 'home' && !isAuthMode && (
          <div className="w-full h-full p-4 md:p-8 overflow-y-auto space-y-8 animate-fadeIn">
            ...
            <div className={`border rounded-3xl p-6 md:p-10 text-center transition-all duration-300 ${
              ...
              <h1 className={`text-xl md:text-3xl font-black tracking-tight mb-4 ${darkMode ? 'text-red-400' : 'text-slate-950'}`}>ستاد دیجیتال فرماندهی و مدیریت همیار بحران</h1>
              ...
              <div className="flex flex-wrap justify-center gap-3 mt-6">
                ...
                <button type="button" onClick={() => navigateToView('volunteer')} className="bg-gradient-to-r from-pink-500 to-blue-500 text-white font-black text-xs px-6 py-2.5 rounded-xl shadow-lg transition transform hover:scale-105 active:scale-95">تکمیل فرم داوطلبی همیاری</button>
              </div>
            </div>

            {/* 🟢 بخش کامپوننت دوقلوی آمار لحظه‌ای و زنده سیستم */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-5xl mx-auto">
              
              {/* کادر اول: آمار زنده ثبت‌نام داوطلبان */}
              <div className={`border rounded-3xl p-5 md:p-6 flex flex-col sm:flex-row items-center gap-4 transition-all duration-300 ${
                ...
                <div className="text-center sm:text-right flex-1 space-y-1">
                  ...
                  <p className={`text-xs md:text-sm font-semibold leading-relaxed ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                    ...
                    <button 
                      onClick={() => navigateToView('volunteer')} 
                      className="inline-block font-black text-emerald-500 hover:text-emerald-400 transition-colors mr-1.5 focus:outline-none focus:underline"
                    >
                      ( فرم داوطلبی )
                    </button>
                  </p>
              ...
              {/* کادر دوم: آمار زنده فوریت‌های بحران و حوادث */}
              <div className={`border rounded-3xl p-5 md:p-6 flex flex-col sm:flex-row items-center gap-4 transition-all duration-300 ${
                darkMode 
                ...
                <div className="w-12 h-12 rounded-2xl bg-red-500/10 border border-red-500/30 flex items-center justify-center text-red-500 shrink-0 shadow-inner">
                ...
                <div className="text-center sm:text-right flex-1 space-y-1">
                  ...
                  <p className={`text-xs md:text-sm font-semibold leading-relaxed ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                    ...
                    <button 
                      ...
                      className="inline-block font-black text-red-500 hover:text-red-400 transition-colors mr-1.5 focus:outline-none focus:underline"
                    >
                      ( فرم ثبت حادثه )
                    </button>
                  </p>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className={`border p-6 rounded-2xl space-y-3 transition shadow-md ${darkMode ? 'bg-red-955/40 border-red-900/60 text-red-50' : 'bg-pink-50 border-pink-200 text-slate-900 shadow-xl'}`}>
                  ...
                  <p className="text-xs md:text-sm leading-relaxed font-semibold opacity-90">مردم و ناظران عینی حادثه را روی نقشه ماهواره‌ای «نشان» علامت‌گذاری کرده و پس از دریافت رمز تایید پیامکی بله، عمق فاجعه را به سامانه همیار بحران گزارش می دهند.</p>
                </div>
                <div className={`border p-6 rounded-2xl space-y-3 transition shadow-md ${darkMode ? 'bg-red-955/40 border-red-900/60 text-red-50' : 'bg-blue-50 border-blue-200 text-slate-900 shadow-xl'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className={`border p-5 rounded-3xl space-y-3 shadow-md ${darkMode ? 'bg-gradient-to-br from-red-955/30 to-slate-955 border-red-900/40' : 'bg-white border-red-200'}`}>
              ...
              <form onSubmit={handleIncidentSubmit} className={`max-w-3xl mx-auto border rounded-3xl p-4 md:p-6 shadow-2xl space-y-4 transition-colors ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-300 text-slate-900'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="flex items-end">
                    <button type="button" onClick={() => handleSendReportOTP(reporterPhone)} className="w-full bg-red-600/10 border border-red-500/30 text-red-600 text-xs font-black h-[38px] rounded-xl hover:bg-red-600/20 transition shadow-sm">ارسال کد پیامکی</button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="relative">
                    ...
                    {openCrisisDropdown && (
                      <div className={`absolute left-0 right-0 mt-1 rounded-xl shadow-2xl border z-50 overflow-hidden animate-fadeIn ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
                ...
                <div>
                  ...
                  <textarea rows={3} required value={incidentDesc} onChange={(e) => { setIncidentDesc(e.target.value); if (e.target.value.length < 10) { setIncidentDesc(e.target.value); } else { setIncidentDesc(e.target.value); } }} onInvalid={(e) => { if ((e.target as HTMLTextAreaElement).value.length === 0) { (e.target as HTMLTextAreaElement).setCustomValidity('لطفاً شرح حادثه و وضعیت مصدومین را پر کنید.'); } }} placeholder="" className={`w-full border rounded-xl px-4 py-2 text-xs font-semibold focus:outline-none resize-none ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-slate-50 border-slate-300 text-slate-900'}`} />
                </div>

                <button type="submit" className="w-full text-xs font-black bg-gradient-to-r from-red-600 to-red-500 text-white rounded-xl py-3 transition shadow-md flex items-center justify-center gap-2"><ShieldAlert className="w-4 h-4" /> <span>ارسال فوری گزارش به ستاد فرماندهی</span></button>
              </form>
            </div>
          </div>
        )}
          ...
          <div className="w-full h-full p-4 md:p-8 overflow-y-auto">
            <form onSubmit={handleVolunteerSubmit} className={`max-w-3xl mx-auto border rounded-3xl p-4 md:p-6 space-y-5 shadow-2xl transition-colors ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-300 text-slate-900'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <div className="flex gap-1.5">
                    ...
                    <button type="button" onClick={() => handleSendVolOTP(volPhone)} className="bg-red-500 text-white text-[10px] font-bold px-3 rounded-xl hover:bg-red-600 shrink-0 shadow-sm">ارسال‌کد</button>
                  </div>
                </div>
                <div>
              ...
              <div className="space-y-2">
                <button type="submit" onClick={() => { if(!isVolPhoneVerified) { alert("❌ خطا: جهت ثبت درخواست ابتدا باید شماره همراه خود را احراز هویت کنید."); } }} className="w-full text-xs font-black bg-gradient-to-r from-red-600 to-red-600 text-white rounded-xl py-3 transition shadow-md"><UserPlus className="w-4 h-4 inline-block ml-1" /> <span>ثبت نهایی درخواست عضویت نیروها</span></button>
                {volSubmitErrorMsg && (
        ...
        {currentView === 'admin' && (isAdminLoggedIn || isHamyaatLoggedIn) && !isAuthMode && (
          <div className="w-full h-full p-4 md:p-6 overflow-y-auto space-y-6">
              ...
              <div className={`border rounded-3xl p-5 shadow-2xl space-y-4 animate-fadeIn ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-red-50 border-red-200 text-slate-900'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  <div>
                    ...
                    <input type="text" value={selectedVolForPage.fullName || ''} onChange={(e) => setSelectedVolForPage({...selectedVolForPage, fullName: e.target.value})} className={`w-full border rounded-xl px-3 py-2 text-xs focus:outline-none font-bold ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300'}`} />
                  </div>
                  <div>
            ...
            {isHamyaatLoggedIn && loggedInHamyar && !selectedVolForPage && (
            ...
            <div className={`mt-4 rounded-3xl p-6 w-full max-w-sm space-y-5 border transition-all ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-300 shadow-2xl' : 'bg-white border-slate-300 shadow-xl text-slate-900'}`}>
              <div className={`flex justify-between items-center border-b pb-4 gap-4 ${darkMode ? 'border-slate-800' : 'border-slate-200'}`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <a href={`tel:${adminProfile.phone}`} style={{ direction: 'ltr', unicodeBidi: 'bidi-override' }} className="dir-ltr inline-block text-left font-mono font-black text-sm md:text-base tracking-widest text-red-600 hover:underline">
                    +989912201633
                  </a>
                </div>
              </div>
              <div className="flex justify-between items-center gap-4 pt-1 font-black">
                ...
                <div className="flex items-center gap-2 max-w-[65%]">
                  ...
                  <a href={`mailto:${adminProfile.email}`} className="font-mono text-[11px] md:text-xs text-red-600 tracking-wide break-all hover:underline">
                    {adminProfile.email}
                  </a>
                </div>
              </div>
        ...
        {/* 🔐 لایه جدید: فرم تاکتیکال احراز هویت و ورود همیاران/مدیریت */}
            ...
            <form onSubmit={handleAdminAuth} className={`w-full max-w-md border rounded-3xl p-6 space-y-4 shadow-2xl transition-colors ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-300 text-slate-900'}`}>
              ...
              {authMethod === 'password' ? (
                ...
                <div className="space-y-3">
                  {!authOtpSent ? (
                    <button type="button" onClick={handleSendAuthOTP} className="w-full bg-red-500/10 border border-red-500/30 text-red-500 text-xs font-black py-2 rounded-xl hover:bg-red-500/20 transition">ارسال رمز یکبار مصرف</button>
                  ) : (
